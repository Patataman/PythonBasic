...
'''
	Creamos 2 magickarps (a falta demás personajes), uno que se mueva
	y otro que no.
'''

magickarp_jugador = Personaje()  #Creación del personaje
grupo_movimiento = pygame.sprite.GroupSingle()
grupo_movimiento.add(magickarp_jugador)

magickarp_obstaculo = Personaje()  #Creación del personaje obstaculo
magickarp_obstaculo.rect.center = (ventana.get_width()-100, ventana.get_height()/2)
grupo_obstaculo = pygame.sprite.GroupSingle()
grupo_obstaculo.add(magickarp_obstaculo) 
...
while True:     #Bucle de "Juego"
    for event in pygame.event.get():    #Cuando ocurre un evento...
        ...
        if event.type == pygame.KEYDOWN:
            ...
            #Cuando ocurre un evento y están colisionando, cambia de posición al otro magickarp. Lo correcto
            #para que quede mejor, habría que comprobar cuando hay un evento, si no, siempre.
            if pygame.sprite.collide_mask(magickarp_jugador, magickarp_obstaculo):
            	magickarp_obstaculo.rect.center = (ventana.get_width()/2+int(random.uniform(-200,200)),
